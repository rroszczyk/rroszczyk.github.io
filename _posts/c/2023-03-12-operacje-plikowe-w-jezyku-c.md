...
comments: false
description: >
   Systemy informatyczne często wykorzystują pliki do przechowywania danych. Dzięki temu przewodnikowi poznasz obsługę plików w&nbsp;języku&nbsp;C.
...
Otwieranie pliku odbywa się za pomocą funkcji *fopen()*{:.maroon} zdefiniowanej w&nbsp;pliku nagłówkowym *stdio.h*.
...
Do odczytu zawartości pliku binarnego należy użyć funkcji *fread()*{:.maroon}. Funkcja ta przyjmuje analogiczne parametry jak wcześniej opisywana funkcja *fwrite()*{:.maroon}:
{: .text-justify}
...
#### Przewijanie na początek pliku

Czasami zachodzi potrzeba przestawienia wskaźnika pozycji pliku na początek. Do tego celu może być przydatna instrukcja *rewind()*{:.maroon}, dzięki temu poleceniu przestawiony zostaje wskaźnik pozycji operacji plikowych na sam początek pliku.
{: .text-justify}

## Pliki tymczasowe

Plik tymczasowy, jest rodzajem pliku stworzonym do przechowywania informacji tymczasowo podczas gdy są one potrzebne do wykonania określonego zadania. Po zakończeniu zadania, plik tymczasowy jest zazwyczaj usuwany przez aplikację, która go stworzyła, lub przez system operacyjny.
{: .text-justify}

W języku C dostępna jest funkcja tempfile przydatna do tworzenia plików tymczasowych:
{: .text-justify}

- zdefiniowana została w pliku nagłówkowym *stdio.h*,
- plik tymczasowy automatycznie jest usuwany po zakończeniu działania programu,
- tworzony plik jest otwierany w trybie binarnym do zapisu i odczytu ***wb+***.

Przykładowy program wykorzystujący plik tymczasowy przedstawiony jest poniżej:
...
{ 
   ...
   printf("Plik tymczasowy został prawidłowo utworzony\n"); 
   while (str[i] != '\0') 
   { 
      fputc(str[i], tmp); 
      i++; 
...
Pliki tymczasowe zazwyczaj znajdują się w specjalnych folderach przeznaczonych dla nich przez system operacyjny, takich jak */tmp*{:.blue} w&nbsp;systemach Unix i&nbsp;Linux, lub folder *Temp*{:.blue} w&nbsp;systemie Windows. Choć są one z&nbsp;założenia tymczasowe, niektóre pliki mogą pozostać na dysku, jeśli proces, który miał je usunąć, zostanie nieoczekiwanie przerwany. W&nbsp;systemach Unixowych pliki te mogą zostać później usunięte przez specjalny mechanizm czyszczenia katalogów zawierających tego typu pliki.
{: .text-justify}
